...
document.addEventListener('DOMContentLoaded', () => {
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            updateUIOnLogout();
            clearNotes(); // Clear notes on logout for privacy
        }
    });
});
...
// Function to clear notes from local storage and UI
function clearNotes() {
    flashNotes = [];
    localStorage.removeItem('flashNotes');
...
function displayFlashNotes(notes) {
    ...
    notes.forEach(note => {
        ...
        if (note.isEditing) {
            // Display in edit mode with placeholder
            noteElement.innerHTML = `
                <input type="text" class="note-title-input" oninput="updateNoteTitle('${note.id}', this.value)" placeholder="Enter title here" required value="${note.title}" />
                ...
                <div contenteditable="true" class="note-content-input" onfocus="removePlaceholder(this)" onblur="addPlaceholder(this)">${note.content}</div>
...
document.getElementById('create-note')?.addEventListener('click', () => {
    ...
    setTimeout(() => {
        const noteElement = document.getElementById(newNote.id);
        noteElement.querySelector('.note-title-input').focus();
    }, 0);
});
...
window.saveEdit = function(noteId) {
    ...
    if (note) {
        ...
        note.content = contentInput.innerHTML.trim(); // Save formatted content
